...
pub fn recursive_count(n: u64, iteration: u64, map: &mut FxHashMap<(u64, i32), u64>, max_iter: u64) -> u64 {
    ...
    let result = if n == 0 {
        rust_nrjv_aoc_2024::day11::recursive_count(1, iteration + 1, map, max_iter)
    } else {
        let digits = count_digits(n);
        if digits % 2 == 0 {
            let (left, right) = split_number_in_half(n, digits);
            rust_nrjv_aoc_2024::day11::recursive_count(left, iteration + 1, map, max_iter) + rust_nrjv_aoc_2024::day11::recursive_count(right, iteration + 1, map, max_iter)
        } else {
            rust_nrjv_aoc_2024::day11::recursive_count(n * 2024, iteration + 1, map, max_iter)
        }
    };
...
fn main() {
    ...
    while let Some((n, iteration)) = stack.pop() {
        total += rust_nrjv_aoc_2024::day11::recursive_count(n, iteration, &mut map, 25);
    }
